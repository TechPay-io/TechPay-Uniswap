...
import './interfaces/IWTPC.sol';

// TechPayUniswapV2Router is a port of the Uniswap V2 router contract
// to the TechPay Photon block chain.
contract TechPayUniswapV2Router is IUniswapV2Router {
    // define used libs
    ...
    // wTPC is the address of the wrapped TPC ERC20 contract,
    // wTPC is a wrapped implementation of the native TechPay Photon token.
    address public wTPC;
    ...
    constructor(address _factory, address _wTPC) public {
        factory = _factory;
        wTPC = _wTPC;
    }

    // payable fallback function verifies that the contract receives native
    // tokens only from the wTPC contract as an unwrap callback.
    function() external payable {
        assert(msg.sender == wTPC);
    }

    // WETH is a compatibility getter around wrapped native TPC tokens
    // address resolution.
    ...
    // addLiquidityETH adds received amount of TechPay Photon native TPC tokens
    // to the specified token pair using wrapper contract to wrap native TPCs first.
    function addLiquidityETH(
        ...
        address pair = UniswapV2Library.pairFor(factory, token, wTPC);
        SafeERC20.safeTransferFrom(IERC20(token), msg.sender, pair, amountToken);

        // swap received TPC before posting it as ERC20
        require(0 == IWTPC(wTPC).deposit.value(amountETH)(), "WTPC: deposit failed");
        assert(IWTPC(wTPC).transfer(pair, amountETH));
    ...
    // Uniswap pair and redeems user's share tokens, the wrapped TPC tokens
    // is swapped back to native TPCs before sending.
    function removeLiquidityETH(
        ...
        // unwrap TPC and send the native
        require(0 == IWTPC(wTPC).withdraw(amountETH), "WTPC: withdraw failed");
    ...
    // with the wrapped native TPC tokens using given permit.
    ...
    // swapExactETHForTokens swaps specified amount of native TPC tokens for the target tokens
    ...
    function swapExactETHForTokens(
        ...
        require(path[0] == wTPC, 'UniswapV2Router: INVALID_PATH');
        ...
        require(0 == IWTPC(wTPC).deposit.value(amounts[0])(), "WTPC: deposit failed");
        assert(IWTPC(wTPC).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));

        // perform the swap
        _swap(amounts, path, to);
    }

    // swapTokensForExactETH swaps specified token for exact value of native TPC tokens
    ...
    function swapTokensForExactETH(
        ...
        // make sure the final step is the wrapped native TPC token
        require(path[path.length - 1] == wTPC, 'UniswapV2Router: INVALID_PATH');
        ...
        require(0 == IWTPC(wTPC).withdraw(amounts[amounts.length - 1]), "WTPC: withdraw failed");
    ...
    // amount of native TPC tokens using pairs specified by the swap path.
    function swapExactTokensForETH(
        ...
        // unwrap to native TPC and send the amount to recipient
        require(0 == IWTPC(wTPC).withdraw(amounts[amounts.length - 1]), "WTPC: withdraw failed");
        Address.sendValue(to.toPayable(), amounts[amounts.length - 1]);
    }

    // swapETHForExactTokens swaps native TPC tokens for exact amount of output tokens
    ...
    function swapETHForExactTokens(
        ...
        // make sure the entry token is wrapped TPC
        require(path[0] == wTPC, 'UniswapV2Router: INVALID_PATH');
        ...
        // wrap incoming TPC first and than deposit to the first swap pair
        require(0 == IWTPC(wTPC).deposit.value(amounts[0])(), "WTPC: deposit failed");
        assert(IWTPC(wTPC).transfer(UniswapV2Library.pairFor(factory, path[0], path[1]), amounts[0]));
